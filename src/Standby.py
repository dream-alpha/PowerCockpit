...
from .RecordingUtils import isRecording, isRecordingOrRecordingSoon, startTimeshiftRecording, stopTimeshiftRecording
...
class Standby(Screen):
	...
	def __onFirstExecBegin(self):
		...
		logger.debug("stopTimeshiftRecording")
		stopTimeshiftRecording()
		is_recording = isRecordingOrRecordingSoon(self.session)
		...
		if not is_recording and hasattr(config.plugins, "moviecockpit"):
			...
			FileManager.getInstance().purgeTrashcan(int(config.plugins.moviecockpit.trashcan_retention.value))
			FileManager.getInstance().archive()
		self.shutdown_timer.start(1000, True)

	def doShutdown(self):
		logger.info("jobs: %s", len(job_manager.getPendingJobs()))
		is_recording = isRecordingOrRecordingSoon(self.session)
